...
import './Home.css';
import inventoryService, { ProductStock } from '../services/inventoryService';
...
export default function Home() {
  ...
  const [productos, setProductos] = useState<ProductStock[]>([]);
  ...
  const loadInventory = () => {
    setProductos(inventoryService.getAllProducts());
    setInventoryStats(inventoryService.getInventoryStats());
  };
  ...
  // Verificar alertas de stock bajo al cargar productos
  useEffect(() => {
    if (productos.length > 0) {
      const lowStockProducts = inventoryService.getLowStockProducts();
      const outOfStockProducts = inventoryService.getOutOfStockProducts();
      
      // Mostrar alertas solo para administradores
      const userType = localStorage.getItem('userType');
      if (userType === 'administrador') {
        if (outOfStockProducts.length > 0) {
          showMessage(`⚠️ ${outOfStockProducts.length} producto(s) sin stock disponible`);
        } else if (lowStockProducts.length > 0) {
          showMessage(`📦 ${lowStockProducts.length} producto(s) con stock bajo`);
        }
      }
    }
  }, [productos]);
  ...
  const addToCart = (productIndex: number) => {
    ...
    if (!inventoryService.isProductAvailable(product.id, currentQty + 1)) {
      showMessage(`Stock insuficiente para ${product.nombre}. Stock disponible: ${product.stock}`);
      return;
    }
  ...
  const incrementQuantity = (id: number) => {
    ...
    if (!inventoryService.isProductAvailable(id, currentQty + 1)) {
      showMessage(`Stock insuficiente. Stock disponible: ${product?.stock || 0}`);
      return;
    }
  ...
  const goToCheckout = () => {
    ...
    for (const item of cart) {
      const product = inventoryService.getProductById(item.id);
      const currentStock = product ? product.stock : 0;
      if (currentStock < item.qty) {
        stockIssues.push(`${item.nombre}: Solo quedan ${currentStock} unidades disponibles`);
      }
    }

    if (stockIssues.length > 0) {
      showMessage(`Stock insuficiente: ${stockIssues.join(', ')}`);
      return;
    }

    // Reducir stock automáticamente al procesar la compra
    try {
      cart.forEach(item => {
        inventoryService.reduceStock(item.id, item.qty);
      });

      showMessage('Compra procesada exitosamente. Stock actualizado.');

      // Navegar a factura con los datos antes de limpiar el carrito
      history.push({
      ...
      setTimeout(() => {
        setCart([]);
        // Recargar productos para actualizar la vista con el nuevo stock
        setProductos(inventoryService.getAllProducts());
      }, 100);

    } catch (error) {
      showMessage('Error al procesar la compra: ' + (error as Error).message);
    }
  };
        ...
        <IonContent className="home-bg ion-padding" fullscreen>
          ...
          >
            {productos.map((producto, index) => {
              ...
              const isLowStock = producto.stock <= producto.stockMinimo && producto.stock > 0;
              
              return (
              <IonCard key={producto.id} className={`producto-card ecommerce ${isOutOfStock ? 'out-of-stock' : ''}`}>
                <div className="img-wrap">
                  <img 
                    ...
                    loading="lazy"
                    style={{ filter: isOutOfStock ? 'grayscale(1) opacity(0.5)' : 'none' }}
                  />
                  {isOutOfStock && (
                ...
                <IonCardContent>
                  ...
                  <div className="stock-info">
                    <span style={{ 
                      ...
                      {isOutOfStock ? 'Sin stock' : `Stock: ${producto.stock} unidades`}
                    </span>
                  </div>
